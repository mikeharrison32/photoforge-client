...
export class CropTool {
  ...
  configure(display: HTMLElement) {
    ...
    this.cropCanvas = new Canvas({width: display.clientWidth, height: display.clientHeight, x: 0, y: 0})
    
    const ctx = this.cropCanvas.getContext("2d") as CanvasRenderingContext2D
    ctx!.fillStyle = "#000000b0"
    
    this.renderCropCanvas(ctx)
    const trCorner = new Corner(display, 10, 3, 0, 0)
    trCorner.elem?.classList.add("crop-tool-corner")

    new MouseDragEvent(trCorner.elem!, true,(e: any) => {
      trCorner.moveTo(e.x / parseFloat(display.style.scale || "1"), e.y / parseFloat(display.style.scale || "1"))
      this.renderCropCanvas(ctx!)
      ctx?.clearRect(e.x / parseFloat(display.style.scale || "1"), 0, 500*2,500*2 )
    ...
    new MouseDragEvent(tlCorner.elem!, true,(e: any) => {
      tlCorner.moveTo(e.x / parseFloat(display.style.scale || "1"), e.y / parseFloat(display.style.scale || "1"))
    ...
    new MouseDragEvent(brCorner.elem!, true,(e: any) => {
      brCorner.moveTo(e.x / parseFloat(display.style.scale || "1"), e.y / parseFloat(display.style.scale || "1"))
    })

    const blCorner = new Corner(display, 10, 3, display.clientWidth - 200, display.clientHeight - 200)
    ...
    new MouseDragEvent(blCorner.elem!, true,(e: any) => {
      blCorner.moveTo(e.x / parseFloat(display.style.scale || "1"), e.y / parseFloat(display.style.scale || "1"))
    })
  ...
  renderCropCanvas(cropCanvasCtx: CanvasRenderingContext2D){
    cropCanvasCtx.clearRect(0, 0, cropCanvasCtx.canvas.clientWidth, cropCanvasCtx.canvas.clientHeight)
    cropCanvasCtx.fillStyle = "#000000b0"
    cropCanvasCtx.fillRect(0, 0, cropCanvasCtx.canvas.clientWidth, cropCanvasCtx.canvas.clientHeight)
  }
}
...
class Corner {
  elem?: HTMLElement
  parent?: HTMLElement
  constructor(parent: HTMLElement,width: number, height: number, x: number, y: number){
